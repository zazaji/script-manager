...
- ⌨️ **快捷启动（Spotlight/Alfred 风格）**：按下 `Option+Space` 呼出全局浮层，即时搜索、过滤并启动脚本、macOS 应用或 AI 智能体。按 `Enter` 在系统终端运行，按 `Cmd+Enter` 在应用内运行。支持拼音模糊搜索。
- 🤖 **AI 智能体集成**：从启动栏通过自定义前缀（如 `>`、`/`、`@`）触达 AI 智能体。支持流式对话、历史记录及独立的全局快捷键。
...
ScriptManager 内置了 **Spotlight/Alfred 风格的全局启动浮层**，让你可以即时访问脚本、应用和 AI 智能体。

- **呼出**：在任何界面按下 `Option+Space`（即使 ScriptManager 在后台运行）。
- **搜索**：输入关键字即可自动过滤脚本、macOS 应用和 AI 任务。支持拼音模糊匹配。
- **运行**：`Enter` 在系统终端（Terminal.app 或 iTerm2）中执行，`Cmd+Enter` 在应用内置 PTY 终端中执行。
- **AI 智能体**：输入智能体前缀（如 `>`、`/`、`@`）即可与 AI 进行流式对话。
- **Tab 自动补全**：按 `Tab` 将选中项的名称自动填入搜索框。
...
致对源码感兴趣的开发者：
- **全局快捷启动**：基于 Carbon 的 `RegisterEventHotKey` API 实现 `Option+Space` 全局快捷键（后台运行也可响应）。使用 `NSPanel` 在 `.popUpMenu` 层级展示磨砂玻璃浮层，并智能跟随鼠标所在屏幕定位。
...
- **拼音模糊搜索**：基于 `CFStringTransform` 实现拼音模糊匹配，从启动栏快速定位脚本和应用。
